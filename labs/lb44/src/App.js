...
function fetchIngredient(type, ingredient) {
  let url = server_url + "/" + type + "/" + ingredient;
...
function fetchIngredients(type) {
  let url = server_url + "/" + type;
  let ingredients = safeFetchJson(url);
  ...
  useEffect(() => {
    types.forEach(
      type => fetchIngredients(type)
        ...
        .then(ingredients => Object.assign({}, ...ingredients.map((ingredient) => ({ [ingredient.name]: ingredient.properties }))))
        .then(ingredients => setInventory(oldState => ({ ...oldState, ...ingredients })))
    );
  }, []);
